...
def main(
    ...
    # LookAtTask. This way, regularization toward the starting pose never sacrifices end-effector
    # tracking; it only uses the redundant degrees of freedom that the LookAtTask leaves free.
    ...
    for name in model_data.ee_names:
        ...
        frame_task = LookAtTask(oink, scene, goal, task_options)
    ...
    def control_loop():
        ...
        display_period = max(dt, 1.0 / 15.0)
        last_display = 0.0
        while running:
            ...
            if not paused:
                with scene_lock:
                    ...
                    # Marker targets are in the world frame, but each LookAtTask expects its
                    ...
                    if reference_filter_tau > 0 and False:
                    ...
                    if limit_acceleration and False:
                    ...
                    q_current = q_current + delta_q_full
