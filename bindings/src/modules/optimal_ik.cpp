...
void init_optimal_ik(nanobind::module_& m) {
  ...
  nanobind::class_<FrameTask, Task>(m, "FrameTask",
      ...
      .def(nanobind::init<const Oink&, const CartesianConfiguration&, const FrameTaskOptions&>(),
           "oink"_a, "target_pose"_a, "options"_a = FrameTaskOptions{})
  ...
  nanobind::class_<Oink>(m, "Oink", "Optimal Inverse Kinematics solver.")
      .def(nanobind::init<Scene&, const std::string&>(), "scene"_a, "group_name"_a,
      ...
      .def(
          ...
          [](Oink& self, const std::vector<std::shared_ptr<Task>>& tasks,
            ...
            auto result = self.solveIk(tasks, constraints, barriers, delta_q, regularization);
            if (!result.has_value()) {
              throw std::runtime_error("IK solve failed: " + result.error());
            }
          },
          "tasks"_a, "constraints"_a, "barriers"_a, "delta_q"_a, "regularization"_a = 1e-12,
      ...
      .def(
          ...
          [](Oink& self, const std::vector<std::shared_ptr<Task>>& tasks,
             nanobind::DRef<Eigen::VectorXd> delta_q, double regularization) {
            auto result = self.solveIk(tasks, delta_q, regularization);
            if (!result.has_value()) {
              throw std::runtime_error("IK solve failed: " + result.error());
            }
          },
          "tasks"_a, "delta_q"_a, "regularization"_a = 1e-12,
      ...
      .def(
          ...
          [](Oink& self, const std::vector<std::shared_ptr<Task>>& tasks,
             const std::vector<std::shared_ptr<Constraints>>& constraints,
             nanobind::DRef<Eigen::VectorXd> delta_q, double regularization) {
            auto result = self.solveIk(tasks, constraints, delta_q, regularization);
            if (!result.has_value()) {
              throw std::runtime_error("IK solve failed: " + result.error());
            }
          },
          "tasks"_a, "constraints"_a, "delta_q"_a, "regularization"_a = 1e-12,
      ...
      .def(
          ...
          [](Oink& self, const std::vector<std::shared_ptr<Task>>& tasks,
             const std::vector<std::shared_ptr<Barrier>>& barriers,
             nanobind::DRef<Eigen::VectorXd> delta_q, double regularization) {
            auto result = self.solveIk(tasks, barriers, delta_q, regularization);
            if (!result.has_value()) {
              throw std::runtime_error("IK solve failed: " + result.error());
            }
          },
          "tasks"_a, "barriers"_a, "delta_q"_a, "regularization"_a = 1e-12,
      ...
      .def(
          ...
          [](Oink& self, const std::vector<std::shared_ptr<Barrier>>& barriers,
            ...
            auto result = self.enforceBarriers(barriers, delta_q, tolerance);
            if (!result.has_value()) {
              throw std::runtime_error("Barrier enforcement failed: " + result.error());
            }
          },
          "barriers"_a, "delta_q"_a, "tolerance"_a = 0.0,
