...
LookAtTask::LookAtTask(const Oink& oink, const Scene& scene,
                       const CartesianConfiguration& target_pose, const LookAtTaskOptions& options)
...
tl::expected<void, std::string> LookAtTask::computeError(const Scene& scene) {
  ...
  Eigen::Vector3d look_at_axis = transform_world_to_frame.rotation() * axis;
  look_at_axis.normalize();

  Eigen::Vector3d frame_to_target =
      transform_world_to_target.translation() - transform_world_to_frame.translation();
  frame_to_target.normalize();

  Eigen::Matrix3d rotation_error =
      Eigen::Quaterniond::FromTwoVectors(look_at_axis, frame_to_target).toRotationMatrix();

  Eigen::Vector3d e_rot = transform_world_to_frame.rotation() * pinocchio::log3(rotation_error);

  error_container.head<3>() = Eigen::Vector3d{0.0, 0.0, 0.0};
  error_container.tail<3>() = e_rot;

  std::cout << error_container << std::endl;
...
Eigen::MatrixXd LookAtTask::createWeightMatrix(double position_cost, double orientation_cost) {
